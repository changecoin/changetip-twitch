    ...
    def changetip_sender(self, channel, sender, receiver, message):
        ...
        if response.get("error_code") == "invalid_sender":
            ...
            if tip["status"] == "out for delivery":
                out += "<3 @%s Tip received from @%s for %s. Collect it by connecting your Twitch account here %s %s" % (tip["receiver"].capitalize(), sender.capitalize(), tip["amount_display"], "➔".decode('utf-8'), tip["collect_url_short"])
