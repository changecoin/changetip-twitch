    ...
    def changetip_sender(self, channel, sender, receiver, message):
        ...
        if response.get("error_code") == "invalid_sender":
            ...
            if tip["status"] == "out for delivery":
                out += u"<3 @%s Tip received from @%s for %s. Collect it by connecting your Twitch account here ➔ %s" % (tip["receiver"].capitalize(), sender.capitalize(), tip["amount_display"], tip["collect_url_short"])
